...
for site in ${sites[@]}; do
	...
	wget -N ${DL_HOST}/${DL_PATH}/${site}.xml -P ${BASEDIR}/docs/${DL_PATH}/${SUB_PATH}
done
...
for site in ${sites[@]}; do
	echo ${site}
	SUB_PATH=`echo ${site} | perl -nle 'print $& if m{.*/}'`
	cat ${BASEDIR}/docs/${DL_PATH}/${site}.xml | perl -nle 'print $& if m{(?<=<sdk:url>).*(?=</sdk:url>)}' | sed "s~^~${DL_HOST}/${DL_PATH}/${SUB_PATH}~g" | wget -N -P ${DL_PATH}/${SUB_PATH} -c -i -
done
...
echo "name,version,api-level,revision,description,obsolete,windowsSize,windowsSHA1,windowsURL,macosxSize,macosxSHA1,macosxURL,linuxSize,linuxSHA1,linuxURL" \
	> ${BASEDIR}/docs/${DL_PATH}/packages.csv.tmp
for site in ${sites[@]}; do
	# http://www.sagehill.net/docbookxsl/InstallingAProcessor.html#cygwin
	xsltproc ${BASEDIR}/${DL_PATH}/${site}.csv.xsl ${BASEDIR}/docs/${DL_PATH}/${site}.xml >> ${BASEDIR}/docs/${DL_PATH}/packages.csv.tmp
done
mv ${BASEDIR}/docs/${DL_PATH}/packages.csv.tmp ${BASEDIR}/docs/${DL_PATH}/packages.csv
